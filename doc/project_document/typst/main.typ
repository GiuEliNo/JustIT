...
#figure(
  image("vopc_manage_booking_status.png", width: 100%),
)
==== Book Appointment
#figure(
  image("VOPC_Add_booking.png", width: 120%),
)
=== Design Level Diagram

==== Singleton
#figure(
  image("singleton_sessionmanager.png", width: 60%),
)
==== State Pattern
#figure(
  image("state_booking.png", width: 120%),
)
===== State Machine Diagram
#figure(
  image("statemachine_booking.png", width: 70%),
)
==== Factory Method
#figure(
  image("factory_method_report_factory.png", width: 70%),
)

=== Observer
#figure(
  image("observer_pattern.png", width: 60%),
)

== Design patterns
=== Singleton pattern

In JustIT, the Singleton pattern is employed to manage user sessions through a centralized SessionManager. This component acts as the single point of access for the application's state, globally accessible via getInstance(). This architectural choice prevents the anti-pattern of passing session objects repetitively throughout the system, thereby ensuring data consistency and low coupling.

Upon a successful login, the controller generates a new Session object encapsulating the LoggedUser instance. This session is securely registered within the SessionManager's internal map using a unique UUID as a key. By invoking getActiveSession(sessionId), any layer of the application can securely retrieve user data (getLoggedUser) and perform Role-Based Access Control checks (e.g., isClient, isTechnician).

Additionally, the session dynamically handles the context of the current shop via setCurrentShop and getCurrentShop. For Clients, it stores the shop they are currently browsing, whereas for Technicians, it stores their affiliated shop. This provides a Single Source of Truth, allowing the Booking, Review, Notification, and Account controllers to reliably access shop details without duplicating business logic.

Finally, the logout operation explicitly invalidates the UUID and clears the associated shared state from the manager, preventing memory leaks and ensuring that no sensitive information remains accessible after the session ends.

=== State Pattern

In JustIt, the State pattern is applied to manage the complex lifecycle of a Booking.
This pattern localizes and partitions state-specific behaviors into distinct concrete classes (e.g., PendingPaymentState, ConfirmedState) that implement a common BookingState interface.
Each state transition and its associated business logic is encapsulated within its respective class, elevating the execution state to full object status. This structure makes the intent of the code clearer: default illegal operations are handled safely by the interface, and valid transitions are explicitly localized, ensuring that the system can be easily extended with new states without altering the Booking context.

=== Factory Method Pattern

In JustIT, the Factory Method pattern is adopted to manage the polymorphic creation of repair reports through the centralized RepairReportFactory class.

The factory is responsible for providing the appropriate concrete implementation of a repair report based on the outcome of a booking. The selection process is driven by the BookingStatus passed to the create() method alongside the data payload (RepairReportBean). If the provided BookingStatus is not supported by the factory, an exception is thrown, preventing the creation of invalid repair report instances.

The RepairReport interface defines the baseline contract. Concrete implementations encapsulate specific business rules and state: RepairReportCompleted manages metrics such as labor hours, part costs, and total cost calculation, whereas RepairReportRejected handles scenarios like refund management.

This design decouples the client (the Application Controller) from the concrete instantiation logic. The client interacts exclusively with the RepairReportFactory and the RepairReport abstraction, remaining entirely unaware of the specific subclass being instantiated.

Centralizing object creation in the factory improves modularity, maintainability, and scalability, enabling the addition of new types of repair report implementations with minimal impact on the existing codebase.

=== Observer pattern
In JustIT, the Observer pattern is used to handle notifications between components without creating direct dependencies between them.

This pattern is crucial for maintaining state consistency between core business entities (such as Bookings and Reviews) and the notification subsystem, without tightly binding them. 

We introduced an abstract class Subject and an Observer interface. Specialized singleton classes, BookingStatusPublisher and ReviewCreatedPublisher, act as Concrete Subjects.

When an application controller updates a booking status or registers a new review, it updates the state within the respective publisher. These publishers manage a dynamic list of subscribers and automatically invoke notifyObservers() whenever a state change occurs, ensuring that all dependent components react seamlessly.
...
=== Booking an appointment
#figure(
  image("sd_book_an_appointment.png", width: 130%),
)
=== Manage Booking
#figure(
  image("sd_manage_booking.png", width: 130%),
)
== State diagram
=== Booking an appointment
#figure(
...
== Booking payment with unsupported card circuit  - Giulio Rustia
We tested creating a booking and finalizing the payment using a card from an unsupported circuit. The test verifies that a exception is thrown, ensuring unsupported payment circuits are correctly rejected.
