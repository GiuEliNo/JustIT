...
public class NotificationQuery {
    ...
    public static final String INSERT_NOTIFICATION =
            "INSERT INTO Notification(username, shop_id, booking_id, type, message, created_time, read) " +
                    "VALUES (?,?,?,?,?,?,0)";

    public static final String SELECT_BY_USER =
            "SELECT N.id, N.username, N.shop_id, N.booking_id, N.type, N.message, B.state AS booking_status, " +
    ...
    public static final String SELECT_BY_SHOP =
            "SELECT N.id, N.username, N.shop_id, N.booking_id, N.type, N.message, B.state AS booking_status, " +
                    ...
                    "JOIN Shop S ON N.shop_id = S.id " +
                    "LEFT JOIN Booking B ON N.booking_id = B.id " +
                    "WHERE N.shop_id = ? AND (N.type = 'REVIEW_CREATED' " +
                    "OR (N.type = 'BOOKING_STATUS' AND B.state = 'PENDING_CONFIRM')) " +
                    "ORDER BY N.created_time DESC";
}
